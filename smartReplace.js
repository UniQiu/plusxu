...
async function replaceWithSecrets(content, Secrets) {
    ...
    await init_notify(Secrets, content, replacements);
        ...
        if (Secrets.COOKIE_QQYD) {
            replacements.push({ key: "$.getdata(qqreadbodyVal)", value: JSON.stringify(Secrets.COOKIE_QQYD.split("\n")[0]) });
            replacements.push({ key: "$.getdata(qqreadtimeurlVal)", value: JSON.stringify(Secrets.COOKIE_QQYD.split("\n")[1]) });
            replacements.push({ key: "$.getdata(qqreadtimeheaderVal)", value: JSON.stringify(Secrets.COOKIE_QQYD.split("\n")[2]) });
        }
